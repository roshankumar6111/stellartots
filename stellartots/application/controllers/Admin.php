...
class Admin extends CI_Controller
{
    ...
    function meal($param1 = '', $param2 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            $data['fromdate']=$date=date('d-m-y',strtotime($fromdate));
            $todate=$this->input->post('to');
            $data['todate']=$date=date('d-m-y',strtotime($todate));
           $count=1;
           foreach($data as $row){
            if($count<6){
            $this->db->set('meal',$row);
            $this->db->where('id',$count);
            $this->db->update('meal');
            }
            else if($count==6){
              $this->db->set('fromdate',$row);
              $this->db->where('id','1');
              $this->db->update('mealtime');
            }
             else{
              $this->db->set('todate',$row);
              $this->db->where('id','1');
              $this->db->update('mealtime');
             }
             $count = $count+1;
            ...
            redirect(base_url() . 'index.php?admin/meal/', 'refresh');
        }
        if ($param1 == 'do_update') {
//            $data['name']         = $this->input->post('name');
            $data['meal'] = $this->input->post('meal');
            $data['id']   = $this->input->post('id');
            $this->db->set('meal', $data['meal']);
        ...
        $page_data['meal']    = $this->db->get('meal')->result_array();
        $page_data['page_name']  = 'meal';
        $page_data['page_title'] = get_phrase('weekly_meal_plan');
    ...
    function student($param1 = '', $param2 = '', $param3 = '')
    {
     ...
     /****MANAGE PARENTS CLASSWISE*****/
      function parent_bulk_add($param1 = '')
	{
		if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
		...
		if ($param1 == 'import_excel')
		{
			move_uploaded_file($_FILES['userfile']['tmp_name'], 'uploads/parent_import.xlsx');
			// Importing excel sheet for bulk student uploads

			include 'simplexlsx.class.php';
			
			$xlsx = new SimpleXLSX('uploads/parent_import.xlsx');
			...
			foreach( $xlsx->rows() as $r ) 
			{
				...
				{
					if ($i == 0)	    $data['name']			=	$r[$i];
					else if ($i == 1)	$data['email']		    =	$r[$i];
					else if ($i == 2)	$data['password']		=	$r[$i];
					else if ($i == 3)	$data['phone']		    =	$r[$i];
					else if ($i == 4)	$data['address']		=	$r[$i];
    ...
    /****MANAGE TEACHERS*****/
    function teacher_bulk_add($param1 = '')
	{
		if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
		...
		if ($param1 == 'import_excel')
		{
			move_uploaded_file($_FILES['userfile']['tmp_name'], 'uploads/teacher_import.xlsx');
			// Importing excel sheet for bulk student uploads

			include 'simplexlsx.class.php';
			
			$xlsx = new SimpleXLSX('uploads/teacher_import.xlsx');
			...
			foreach( $xlsx->rows() as $r ) 
			{
			    ...
			    $data['birthday'] = ($data['birthday'] - 25569) * 86400;
                $data['birthday']= gmdate("d-m-Y", $data['birthday']);
...
/*************Weekly progresses Report *****************************/
/************ end Weekly progresses Report **************************/

/****** Weekly exam controller **********************/
     /*************** manage exam controller ********************/
      function weekly_exam($param1 = '', $param2 = '' , $param3 = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
        if ($param1 == 'create') {
            ...
            $this->db->insert('weekly_exam', $data);
            $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
            redirect(base_url() . 'index.php?admin/weekly_exam/', 'refresh');
        }
        if ($param1 == 'edit' && $param2 == 'do_update') {
            ...
            $this->db->update('weekly_exam', $data);
            $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
            redirect(base_url() . 'index.php?admin/weekly_exam/', 'refresh');
        } else if ($param1 == 'edit') {
        ...
        if ($param1 == 'delete') {
            ...
            $this->db->delete('weekly_exam');
            $this->session->set_flashdata('flash_message' , get_phrase('data_deleted'));
            redirect(base_url() . 'index.php?admin/weekly_exam/', 'refresh');
        }
        $page_data['exams']      = $this->db->get('weekly_exam')->result_array();
        $page_data['page_name']  = 'weekly_exam';
...
//                    $message      .= $row2['name'] . $subject . ' : ' . $mark_obtained . ' , ';
//                    
//                }
//                // send sms
...
//
    /****MANAGE EXAM MARKS*****/
    function weekly_exam_marks($exam_id = '', $class_id = '', $subject_id = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
        ...
        if ($this->input->post('operation') == 'selection') {
            ...
            if ($page_data['exam_id'] > 0 && $page_data['class_id'] > 0 && $page_data['subject_id'] > 0) {
                redirect(base_url() . 'index.php?admin/weekly_exam_marks/' . $page_data['exam_id'] . '/' . $page_data['class_id'] . '/' . $page_data['subject_id'], 'refresh');
            } else {
                $this->session->set_flashdata('mark_message', 'Choose exam, class and subject');
                redirect(base_url() . 'index.php?admin/weekly_exam_marks/', 'refresh');
            }
        }
        if ($this->input->post('operation') == 'update') {
            ...
            foreach($students as $row) {
                ...
                $this->db->update('weekly_mark', array('mark_obtained' => $data['mark_obtained'] , 'comment' => $data['comment']));
            }
            $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
            redirect(base_url() . 'index.php?admin/weekly_exam_marks/' . $this->input->post('exam_id') . '/' . $this->input->post('class_id') . '/' . $this->input->post('subject_id'), 'refresh');
        }
        $page_data['exam_id']    = $exam_id;
        $page_data['class_id']   = $class_id;
        $page_data['subject_id'] = $subject_id;
        $page_data['page_info'] = 'Weekly Exam marks';
        $page_data['page_name']  = 'weekly_exam_marks';
        $page_data['page_title'] = get_phrase('manage_exam_marks');
        $this->load->view('backend/index', $page_data);
    }
    
     /****MANAGE GRADES*****/
    function weekly_exam_grade($param1 = '', $param2 = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
        if ($param1 == 'create') {
            ...
            $data['comment']     = $this->input->post('comment');
            $this->db->insert('weekly_exam_grade', $data);
            $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
            redirect(base_url() . 'index.php?admin/weekly_exam_grade/', 'refresh');
        }
        if ($param1 == 'do_update') {
            $data['name']        = $this->input->post('name');
            $data['grade_point'] = $this->input->post('grade_point');
            ...
            $this->db->update('weekly_exam_grade', $data);
            $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
            redirect(base_url() . 'index.php?admin/weekly_exam_grade/', 'refresh');
        } else if ($param1 == 'edit') {
        ...
        if ($param1 == 'delete') {
            $this->db->where('grade_id', $param2);
            $this->db->delete('weekly_exam_grade');
            $this->session->set_flashdata('flash_message' , get_phrase('data_deleted'));
            redirect(base_url() . 'index.php?admin/weekly_exam_grade/', 'refresh');
        }
        $page_data['grades']     = $this->db->get('weekly_exam_grade')->result_array();
        $page_data['page_name']  = 'weekly_exam_grade';
...
/******** weekly progress report**********************/
     function weekly_progress_report( $class_id = '', $report_id='', $student_id='')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
        ...
        if ($this->input->post('operation') == 'selection') {
            ...
            if ($page_data['class_id'] > 0 ) {
                redirect(base_url() . 'index.php?admin/weekly_progress_report/' . $page_data['class_id'] , 'refresh');
            } else {
                $this->session->set_flashdata('mark_message', 'Choose class');
                redirect(base_url() . 'index.php?admin/weekly_progress_report/', 'refresh');
            }
        }
        if ($this->input->post('operation') == 'update') {
            ...
            foreach($students as $row) {
                ...
                $data['monday']        = $this->input->post('monday_' . $row['student_id']);
                $data['tuesday']       = $this->input->post('tuesday_' . $row['student_id']);
                $data['wednesday']       = $this->input->post('wednesday_' . $row['student_id']);
                $data['thursday']       = $this->input->post('thursday_' . $row['student_id']);
                $data['friday']       = $this->input->post('friday_' . $row['student_id']);
                $data['saturday']       = $this->input->post('saturday_' . $row['student_id']);
              
                $this->db->where('student_id', $row['student_id']);
//                $this->db->update('weekly_progress_report', array('monday' => $data['monday'] ,'tuesday'->$data['tuesday'], 'wednesday' => $data['wednesday'],'thursday' => $data['thursday'] , 'friday' => $data['friday']));
                 $this->db->update('weekly_progress_report', $data);
            }
            //date code starts
//            $date['class_id']=$class_id;
            $fromdate= $this->input->post('from_date');
             $date['fromdate']=date('d-m-y',strtotime($fromdate));
            $todate= $this->input->post('to_date');
             $date['todate']=date('d-m-y',strtotime($todate));
            $this->db->where('report_id','1');
            $this->db->update('weekly_progress_report_date',$date);
            //end of date code
            $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
            redirect(base_url() . 'index.php?admin/weekly_progress_report/' . $this->input->post('class_id') . '/'. $row['student_id'], 'refresh');
        }
        $page_data['class_id']   = $class_id;
        $page_data['page_info'] = 'Weekly progress report';
        $page_data['page_name']  = 'weekly_progress_report';
        $page_data['page_title'] = get_phrase('manage_progress_report');
        $this->load->view('backend/index', $page_data);
    }
/******** end weekly progress report**********************/
    ...
    function exam($param1 = '', $param2 = '' , $param3 = '')
    {
     ...
     /****** SEND PROGRESS REPORT VIA SMS ********/
    function weekly_progress_report_sms($param1 = '' , $param2 = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
        ...
        if ($param1 == 'send_sms') {
            ...
            $class_id   =   $this->input->post('class_id');
            $receiver   =   $this->input->post('receiver');
            $data = $this->input->post('students_id_value');
            ...
            foreach ($students as $row) {
                ...
                if ($receiver == 'parent' && $row['parent_id'] != '') 
                    $receiver_phone = $this->db->get_where('parent' , array('parent_id' => $row['parent_id']))->row()->phone;
                $this->db->where('student_id' , $row['student_id']);
                $report = $this->db->get('weekly_progress_report')->result_array();
                $message = '';
              foreach ($report as $row2) {
                    $monday = $row2['monday']; 
                    $tuesday= $row2['tuesday'];
                    $wednesday= $row2['wednesday'];
                    $thursday= $row2['thursday'];
                    $friday= $row2['friday'];
                    $message      .= $row2['name'] . ' Monday_progress: ' . $monday . ' , Tuesday_progress: '. $tuesday. ', Wednesday_progress: '.$wednesday . ', Thursady_progress: '.$thursday.', friday_progress: '.$friday ;
            ...
            $this->session->set_flashdata('flash_message' , get_phrase('message_sent'));
            redirect(base_url() . 'index.php?admin/weekly_progress_report_sms' , 'refresh');
        }
        $page_data['page_name']  = 'weekly_progress_report_sms';
        $page_data['page_title'] = get_phrase('send_progress_report_by_sms');
        $this->load->view('backend/index', $page_data);
    }
